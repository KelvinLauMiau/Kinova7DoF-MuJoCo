...
class JntImpedance:
    def __init__(self, urdf_path: str):
        ...
        # 修改为 8 维，支持 8 个关节
        self.k = 6.0 * np.ones(8)
        self.B = 0.8 * np.ones(8)
    ...
    def compute_jnt_torque(self, q_des, v_des, q_cur, v_cur):
        ...
        :param q_des: desired joint position (8-dim)
        :param v_des: desired joint velocity (8-dim)
        :param q_cur: current joint position (8-dim)
        :param v_cur: current joint velocity (8-dim)
        :return: desired joint torque (8-dim)
        """
...
class Robot:
    # 修改为 8 个 actuator
    ACTUATORS = ['a1', 'a2', 'a3', 'a4', 'a5', 'a6', 'a7', 'a8']

    def __init__(self, control_freq=20) -> None:
        self.mj_model = mujoco.MjModel.from_xml_path(filename='../Model/WithTool/KinovaGen3_7dof_description/KinovaGen3_7dof_description.xml')
        ...
        # 初始化控制器 (拖动示教)
        self.controller = JntImpedance(urdf_path='../Model/WithTool/KinovaGen3_7dof_description/urdf/KinovaGen3_7dof_description.urdf')
        # 初始目标角度设为当前状态（qpos 应为 8-dim）
    ...
    def inner_step(self, action):
        # action 预期为 8-dim 向量
        tau = self.controller.compute_jnt_torque(
            q_des=action,
            v_des=np.zeros(8),
...
if __name__ == '__main__':
    ...
    for _ in range(int(1e5)):
        # 使用当前 qpos 作为目标 action，达到拖动示教效果
